function [wavelet_power_vals frex] = wavelet_power(EEG,varargin);
...
% Edited and tested by John JB Allen and Johannes Rodrigues (johannes.rodrigues@uni-wuerzburg.de)
...
if ~exist('lowfreq','var')              % checking iif it is a variable
    lowfreq = 8;                        % assigning default, and then creating user message
...
%wavelet_power_vals = squeeze(power_collpsed_over_time(:,key_freqs,:));
...
wavelet_power_vals = squeeze(power_not_collpsed_over_time(:,key_freqs,:,:)); 
